...
void Player::Update() {
    ...
    // Not yet
    // glm::ivec3 playerChunkPosition = glm::ivec3(static_cast<int>(entityData.position.x / chunkSize),
    // static_cast<int>(entityData.position.y / chunkSize), static_cast<int>(entityData.position.z / chunkSize)); std::cout <<
    // playerChunkPosition.x << " " << playerChunkPosition.y << " " << playerChunkPosition.z << " " <<
    // chunkHandler.GetChunk(playerChunkPosition.x, playerChunkPosition.y, playerChunkPosition.z).generationStatus << std::endl; if
    // (!chunkHandler.GetChunk(playerChunkPosition.x, playerChunkPosition.y, playerChunkPosition.z).isGenerated) { 	std::cout << "Not
    // Generated!" << std::endl; 	chunkHandler.AddChunk(playerChunkPosition.x, playerChunkPosition.y, playerChunkPosition.z);
...
void Player::MouseButtonCallback() {
    ...
    if (RaycastWorld(entityData.position, entityData.orientation, 500, chunkHandler, blockPosition, chunkPosition, hit)) {
        ...
        if (blockPosition.x == 0 || blockPosition.x == chunkSize - 1 || blockPosition.y == 0 || blockPosition.y == chunkSize - 1 ||
            blockPosition.z == 0 || blockPosition.z == chunkSize - 1) {
...
const std::tuple<int, int, int> Player::GetPosition() {
    return std::make_tuple(
        static_cast<int>(entityData.position.x), static_cast<int>(entityData.position.y), static_cast<int>(entityData.position.z)
    );
}
