...
static int positiveModulo(int a, int b) { return (a % b + b) % b; }
...
bool CollideAxis(short axis, EntityData &newEntityData, ChunkHandler &chunkHandler) {
    ...
    if (!chunkHandler.GetChunk(playerChunkX, playerChunkY, playerChunkZ).isGenerated) {
    ...
    if (axis == 0) {
        ...
        // glm::vec3 min1 = glm::vec3(newEntityData.physicsBoundingBox.corner1.x + newEntityData.position.x,
        // newEntityData.physicsBoundingBox.corner1.y + newEntityData.position.y, newEntityData.physicsBoundingBox.corner1.z +
        // newEntityData.position.z); glm::vec3 max1 = glm::vec3(newEntityData.physicsBoundingBox.corner2.x + newEntityData.position.x,
        // newEntityData.physicsBoundingBox.corner2.y + newEntityData.position.y, newEntityData.physicsBoundingBox.corner2.z +
        // newEntityData.position.z); for (int x = static_cast<int>(std::floor(newEntityData.physicsBoundingBox.corner1.x)); x <=
        // static_cast<int>(std::ceil(newEntityData.physicsBoundingBox.corner2.x)); ++x) { 	for (int y =
        // static_cast<int>(std::floor(newEntityData.physicsBoundingBox.corner1.y)); y <=
        // static_cast<int>(std::ceil(newEntityData.physicsBoundingBox.corner2.y)); ++y) { 		for (int z =
        // static_cast<int>(std::floor(newEntityData.physicsBoundingBox.corner1.z)); z <=
        // static_cast<int>(std::ceil(newEntityData.physicsBoundingBox.corner2.z)); ++z) { 			int targetChunkX =
        // playerChunkX + (playerBlockX + x < 0 ? -1 : playerBlockX + x >= 32 ? 1 : 0); 			int targetChunkY =
        // playerChunkY + (playerBlockY + y < 0 ? -1 : playerBlockY + y >= 32 ? 1 : 0); 			int targetChunkZ =
        // playerChunkZ + (playerBlockZ + z < 0 ? -1 : playerBlockZ + z >= 32 ? 1 : 0);
        //
        ...
        //
        //			if (block.GetType() > 0) {
        //				glm::vec3 min2 = glm::vec3(localBlockX + (targetChunkX * chunkSize), localBlockY + (targetChunkY *
        // chunkSize), localBlockZ + (targetChunkZ * chunkSize)); 				glm::vec3 max2 = min2 + glm::vec3(1.0f);
        //
        //				bool isColliding =
...
static bool ApplyTerrainCollision(EntityData &newEntityData, ChunkHandler &chunkHandler) {
    ...
    newEntityData.globalChunkPosition = glm::ivec3(
        (newEntityData.position.x >= 0) ? static_cast<int>(newEntityData.position.x / 32)
                                        : static_cast<int>((newEntityData.position.x - 31) / 32),
        (newEntityData.position.y >= 0) ? static_cast<int>(newEntityData.position.y / 32)
                                        : static_cast<int>((newEntityData.position.y - 31) / 32),
        (newEntityData.position.z >= 0) ? static_cast<int>(newEntityData.position.z / 32)
                                        : static_cast<int>((newEntityData.position.z - 31) / 32)
    );

    newEntityData.localChunkPosition = glm::ivec3(
        positiveModulo(static_cast<int>(newEntityData.position.x), 32), positiveModulo(static_cast<int>(newEntityData.position.y), 32),
        positiveModulo(static_cast<int>(newEntityData.position.z), 32)
    );
...
bool RaycastWorld(
    const glm::vec3 &origin, const glm::vec3 &direction, int maxDistance, ChunkHandler &chunkHandler, glm::ivec3 &blockHitPosition,
    glm::ivec3 &chunkHitPosition, bool &isHit
) {
    glm::ivec3 currentBlock = glm::floor(origin);
    glm::ivec3 currentChunk = glm::ivec3(
        glm::floor(float(currentBlock.x) / float(chunkSize)), glm::floor(float(currentBlock.y) / float(chunkSize)),
        glm::floor(float(currentBlock.z) / float(chunkSize))
    );
    ...
    glm::vec3 distanceToBlockBoundary = glm::vec3(
        (currentBlock.x + (stepDirection.x > 0 ? 1 : 0) - origin.x) / direction.x,
        (currentBlock.y + (stepDirection.y > 0 ? 1 : 0) - origin.y) / direction.y,
        (currentBlock.z + (stepDirection.z > 0 ? 1 : 0) - origin.z) / direction.z
    );

    for (int i = 0; i < maxDistance; ++i) {
        glm::ivec3 localBlockPos = glm::ivec3(
            positiveModulo(currentBlock.x, chunkSize), positiveModulo(currentBlock.y, chunkSize), positiveModulo(currentBlock.z, chunkSize)
        );
        ...
        if (chunk.isGenerated) {
            ...
            if (!block.IsAir()) {
                blockHitPosition = glm::ivec3(
                    positiveModulo(currentBlock.x, chunkSize), positiveModulo(currentBlock.y, chunkSize),
                    positiveModulo(currentBlock.z, chunkSize)
                );
