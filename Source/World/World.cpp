...
World::World(unsigned int worldSize, SingleplayerHandler *singleplayerHandler)
    : totalChunks(0), totalMemoryUsage(0), singleplayerHandler(singleplayerHandler), worldSize(worldSize), chunkHandler(*this) {
...
void World::GenerateWorld() {
    ...
    INFO(
        "World creation with chunk count of " + std::to_string(chunkCount) + " took " + std::to_string(duration) + " ms" + " or roughly " +
        std::to_string(chunkGenerationSpeed) + " ms per chunk (slightly innacurate as empty chunks are skipped)"
    );
...
void World::GenerateChunk(int chunkX, int chunkY, int chunkZ, Chunk &chunk, bool updateCallerChunk, Chunk &callerChunk) {
    ...
    if (positiveXChunk.isGenerated && negativeXChunk.isGenerated && positiveYChunk.isGenerated && negativeYChunk.isGenerated &&
        positiveZChunk.isGenerated && negativeZChunk.isGenerated && !chunk.isMeshed) {
...
Chunk World::GetChunk(int chunkX, int chunkY, int chunkZ) { return chunkHandler.GetChunk(chunkX, chunkY, chunkZ); }
...
std::vector<float> &World::GetChunkDebugVisualizationVertices() {
    ...
    for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end(); ++it) {
        ...
        chunkDebugVisualizationVertices.insert(
            chunkDebugVisualizationVertices.end(), chunk.GetDebugVisualizationVertices().begin(),
...
std::vector<GLuint> &World::GetChunkDebugVisualizationIndices() {
    ...
    for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end(); ++it) {
        ...
        chunkDebugVisualizationIndices.insert(
            chunkDebugVisualizationIndices.end(), chunk.GetDebugVisualizationIndices().begin(), chunk.GetDebugVisualizationIndices().end()
        );
    }
...
std::vector<glm::vec4> &World::GetChunkOrigins() {
    ...
    for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end(); ++it) {
        auto &chunk = it->second;
        chunkOrigins.emplace_back(glm::vec4(
            chunk.chunkX * chunkSize + static_cast<int>(chunkSize / 2), chunk.chunkY * chunkSize + static_cast<int>(chunkSize / 2),
            chunk.chunkZ * chunkSize + static_cast<int>(chunkSize / 2), chunk.generationStatus
        ));
    }
    return chunkOrigins;
}
