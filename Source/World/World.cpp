...
World::World(unsigned int worldSize, SingleplayerHandler *singleplayerHandler)
    : totalChunks(0), totalMemoryUsage(0),
      singleplayerHandler(singleplayerHandler), worldSize(worldSize),
      chunkHandler(*this) {
...
void World::Setup(Window &window) { player.Setup(window); }

// Currently just calls SetupRenderComponents on all the chunks
void World::SetupRenderComponents() {
  for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end();
       ++it) {
...
void World::GenerateWorld() {
  ...
  INFO("World creation with chunk count of " + std::to_string(chunkCount) +
       " took " + std::to_string(duration) + " ms" + " or roughly " +
       std::to_string(chunkGenerationSpeed) +
       " ms per chunk (slightly innacurate as empty chunks are skipped)");
  INFO("Around " + std::to_string(chunkgenspeed_seconds) +
       " chunks generated, meshed and added per second");
...
void World::GenerateChunk(int chunkX, int chunkY, int chunkZ, Chunk &chunk,
                          bool updateCallerChunk, Chunk &callerChunk) {
  // Basic procedures for preparing a chunk, SetPosition should be done before
  // ANYTHING ELSE, or functions relying on the chunks position will not work
  // properly
  ...
  if (!chunk.isGenerated) {
    // std::cout << "[Debug] GENERATING {" << chunkX << ", " << chunkY << ", "
    // << chunkZ << "}" << std::endl;
  ...
  if (positiveXChunk.isGenerated && negativeXChunk.isGenerated &&
    ...
    if (!positiveZChunk.isGenerated) {
      // std::cout << "[Debug] ==5== Going for {" << chunkX << ", " << chunkY <<
      // ", " << chunkZ + 1 << "} From: {" << chunkX << ", " << chunkY << ", "
      // << chunkZ << "}" << std::endl;
      GenerateChunk(chunkX, chunkY, chunkZ + 1, positiveZChunk, true, chunk);
    }

    if (!negativeZChunk.isGenerated) {
      // std::cout << "[Debug] ==6== Going for {" << chunkX << ", " << chunkY <<
      // ", " << chunkZ - 1 << "} From: {" << chunkX << ", " << chunkY << ", "
      // << chunkZ << "}" << std::endl;
  ...
  if (updateCallerChunk) {
    GenerateChunk(callerChunk.chunkX, callerChunk.chunkY, callerChunk.chunkZ,
                  callerChunk, false, chunk);
  }
}

void World::Update() { player.Update(); }
...
std::vector<float> &World::GetChunkDebugVisualizationVertices() {
  ...
  for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end();
       ++it) {
    auto &chunk = it->second;
    chunkDebugVisualizationVertices.insert(
        chunkDebugVisualizationVertices.end(),
        chunk.GetDebugVisualizationVertices().begin(),
        chunk.GetDebugVisualizationVertices().end());
  }
...
std::vector<GLuint> &World::GetChunkDebugVisualizationIndices() {
  ...
  for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end();
       ++it) {
    auto &chunk = it->second;
    chunkDebugVisualizationIndices.insert(
        chunkDebugVisualizationIndices.end(),
        chunk.GetDebugVisualizationIndices().begin(),
        chunk.GetDebugVisualizationIndices().end());
  }
...
std::vector<glm::vec4> &World::GetChunkOrigins() {
  ...
  for (auto it = chunkHandler.chunks.begin(); it != chunkHandler.chunks.end();
       ++it) {
    auto &chunk = it->second;
    chunkOrigins.emplace_back(
        glm::vec4(chunk.chunkX * chunkSize + static_cast<int>(chunkSize / 2),
                  chunk.chunkY * chunkSize + static_cast<int>(chunkSize / 2),
                  chunk.chunkZ * chunkSize + static_cast<int>(chunkSize / 2),
...
bool World::StartTickThread() {
  if (shouldTick) {
    std::cout << "[World Tick Thread / Info] Tried to start tick thread while "
                 "it was running, aborting"
              << std::endl;
    return false;
  }
...
bool World::StopTickThread() {
  if (!shouldTick) {
    std::cout << "[World Tick Thread / Info] Tried to stop tick thread while "
                 "it was stopped, aborting"
              << std::endl;
    return false;
  }
...
void World::Tick() {
  ...
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                      end_time - tpsStartTime)
                      .count();

  if (duration >= 1000.0) {
    ticksPerSecond = static_cast<unsigned int>(static_cast<float>(totalTicks) /
                                               (duration / 1000.0));
    totalTicks = 0;
