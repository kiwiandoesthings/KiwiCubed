...
struct BlockStringID {
    ...
    bool IsAir() { return strcmp(blockName, "air") == 0; }

    bool operator==(const BlockStringID &other) const { return strcmp(modID, other.modID) == 0 && strcmp(blockName, other.blockName) == 0; }

    std::string CanonicalName() { return std::string(modID) + ":" + blockName; }
};

template <> struct std::hash<BlockStringID> {
...
struct BlockPosition {
    ...
    int xInt() const { return static_cast<int>(x.to_ulong()); }

    int yInt() const { return static_cast<int>(x.to_ulong()); }

    int zInt() const { return static_cast<int>(x.to_ulong()); }
};
...
struct BlockType {
    ...
    // For that a texture-mesher type thing would be needed like in Minecraft to take all of the seperate block textures, and combine them
    // into one atlas.
    ...
    // Also, custom block properties need to be accounted for. If a mod wants to add some custom data to its blocks to be read, how should
    // that be done?

    bool IsAir() { return blockStringID.IsAir(); }
};

class BlockManager {
  public:
    BlockManager() : blockTypes({}) {
        RegisterBlockType({.blockStringID = {"kiwicubed", "air"}, .textures = {0}});
        RegisterBlockType({.blockStringID = {"kiwicubed", "stone"}, .textures = {0, 1, 2, 3}});
    }
...
class Block {
  public:
    ...
    void AddFace(
        std::vector<GLfloat> &vertices, std::vector<GLuint> &indices, FaceDirection faceDirection, int chunkX, int chunkY, int chunkZ,
