...
Shader::Shader(const std::string &vertexFilePath,
  ...
  if (!fragmentSource.empty() || !vertexSource.empty()) {
    shaderProgramID = CreateShader(vertexSource, fragmentSource, vertexFilePath,
                                   fragmentFilePath);
    INFO("Successfully created shader program with ID of " +
...
std::string Shader::ParseShader(const std::string &filePath) {
  ...
  LOG_CHECK_RETURN(file.is_open(),
                   "Successfully opened shader file at " + filePath,
                   "Could not open shader file at " + filePath, "");
  std::string str;
  std::string content;
...
int Shader::CompileShader(unsigned int type, const std::string &source,
  ...
  if (!result) {
    ...
    std::cerr << "[Shader Compilation / Error] Failed to compile "
              << (type == GL_VERTEX_SHADER ? "vertex" : "fragment")
              << " shader from file " << filePath << std::endl;
    std::cerr
        << "[Shader Compilation / Error] Shader compilation error message: "
        << std::endl
        << message << std::endl;
...
int Shader::CreateShader(const std::string &vertexShader,
  ...
  int vertexShaderResult =
      CompileShader(GL_VERTEX_SHADER, vertexShader, vertexFilePath);
  int fragmentShaderResult =
      CompileShader(GL_FRAGMENT_SHADER, fragmentShader, fragmentFilePath);
...
unsigned int Shader::UniformTest(const char *uniform) const {
  ...
  if (uniformLocation == -1) {
    std::cerr << "[Shader Uniform Setting / Error] Could not find uniform: "
              << uniform << " in the specified shader with ID of "
...
void Shader::Bind() const { GLCall(glUseProgram(shaderProgramID)); }

void Shader::Unbind() const { GLCall(glUseProgram(0)); }

void Shader::Delete() const { GLCall(glDeleteProgram(shaderProgramID)); }
