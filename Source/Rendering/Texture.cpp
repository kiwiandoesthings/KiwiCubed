...
void Texture::Bind() const { GLCall(glBindTexture(type, ID)); }

void Texture::Unbind() const { GLCall(glBindTexture(type, 0)); }

void Texture::Delete() const { GLCall(glDeleteTextures(1, &ID)); }