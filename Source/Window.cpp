...
GLFWwindow *Window::CreateWindowInstance(int windowWidth, int windowHeight,
  ...
  if (strcmp(windowType, "fullscreen") == 0) {
    window = glfwCreateWindow(mode->width, mode->height, windowTitle, monitor,
                              nullptr);
  } else if (strcmp(windowType, "window_borderless") == 0) {
    glfwWindowHint(GLFW_DECORATED, GLFW_FALSE);
    window = glfwCreateWindow(mode->width, mode->height, windowTitle, nullptr,
                              nullptr);
  } else if (strcmp(windowType, "window_maximized") == 0) {
    window = glfwCreateWindow(windowWidth, windowHeight, windowTitle, nullptr,
                              nullptr);
...
void Window::Setup() {
  ...
  inputHandler.RegisterKeyCallback(GLFW_KEY_SPACE, [&]() {
    isFocused = true;
    glfwSetCursorPos(window, static_cast<float>(windowWidth) / 2,
...
GLFWwindow *Window::GetWindowInstance() { return window; }
